...
const setter = prop => (state, value) => value === state[prop] ? state : Object.assign({}, state, { [prop]: value });
...
function createModel(name, options) {
  ...
  const ModelClass = {
    [name]: class extends Model {
      ...
      getFullState() {
        const childStates = Reflect.ownKeys(this.children).reduce((state, child) => {
          state[child] = this.children[child].getFullState();
          return state;
        }, {});

        return Object.assign({}, this.state, childStates);
      }
    }
  }[name];
...
function buildInitialState(propTypes, values) {
  ...
  return props.filter(prop => !isModel(propTypes[prop])).reduce((state, prop) => {
...
function buildChildModels(propTypes, values) {
  const props = Reflect.ownKeys(propTypes);
  return props.filter(prop => isModel(propTypes[prop])).reduce((children, prop) => {
...
function buildDefaultSetters(propTypes) {
  const props = Reflect.ownKeys(propTypes);
  return props.filter(prop => !isModel(propTypes[prop])).reduce((setters, prop) => {
