  ...
  return function createTransformsProxy(model, path) {
    return new Proxy(model, {
      get(_, key) {
        ...
        throw new Error(`Unknown transform or child model: ${[...path, key].join('.')}`);
      }
    });
  };
}
...
function updateState(state, path, modelState, updateType) {
  if (path.length === 0) {
    ...
    return Object.assign({}, state, modelState);
  }

  const child = path[0];
  const remainingPath = path.slice(1);
  const newState = Array.isArray(state) ? [...state] : Object.assign({}, state);

  newState[child] = updateState(state[child], remainingPath, modelState, updateType);
  return newState;
}
